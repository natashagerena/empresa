...
function add_to_twig($twig) {
    ...
    $twig->addFunction(new Twig_SimpleFunction('generateFlags', 'generateFlags'));
...
function is_active($pag) {
    ...
    if ( is_array($pag) ) :
        foreach ($pag as $p) :
            if ( $slug == $p ) return "active";
        endforeach;
    else :
        if ( $slug == $pag ) return "active";
    endif;
}
